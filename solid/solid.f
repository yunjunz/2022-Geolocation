      subroutine solid_f(iyr,imo,idy,ihh,imm,iss,
     * glad0,glod0,steplat,steplon,nlat,nlon)

*** Calculate the predicted solid earth tide (SET) for the given date/time and spatial grid
*** Input format:
***     myfile - str, output text filename
...
*** Output format:
***     longitude, latitude, SET_north, SET_east, SET_up

*** Written by Dennis Milbert, 2018-06-01.
***     Code available at: http://geodesyworld.github.io/SOFTS/solid.htm
*** Modified by Simran Sangha and Zhang Yunjun to calculate a spatial grid
***     for a given date/time, 2020-09-06
...
*** input section
      ...
      if(glad0.lt.-90.d0.or.glad0.gt.90.d0) then
        write(*,'(*(G0.9,:,","))') 'error with input minlat=',glad0
        go to 98
      endif

      if(glod0.lt.-360.d0.or.glod0.gt.360.d0) then
        write(*,'(*(G0.9,:,","))') 'error with input minlon=',glod0
        go to 98
      endif
...
*** header
      ...
      write(lout,'(a,2f15.9)') 'N, S = ',glad0,glad1
      write(lout,'(a,2f15.9)') 'W, E = ',glod0,glod1
      write(lout,'(a,f15.9,i9)') 'steplat, nlat = ',steplat,nlat
      write(lout,'(a,f15.9,i9)') 'steplon, nlon = ',steplon,nlon
      write(lout,'(a)') 'lat,lon,set_north,set_east,set_up'
...
*** determine local geodetic horizon components (topocentric)
     ...
     *              iyr,imo,idy,ihr,imn,sec-0.001d0)
        ...
        write(lout,'(*(G0.9,:,","))') glad,glod,ut,vt,wt
